...
class RoomFirestore {
  static final FirebaseFirestore firebaseFirestoreInstance = FirebaseFirestore
      .instance;
  static final _roomCollection = firebaseFirestoreInstance.collection('room');
  ...
  static Future<List<TalkRoom>?> fetchJoinedRooms(QuerySnapshot snapshot) async {
    try {
      ...
      print(myUid);
      final snapshot = await _roomCollection.where('join_user_ids',arrayContains: myUid).get();
      List<TalkRoom> talkRooms = [];
      for (var doc in snapshot.docs) {
        Map<String, dynamic> data = doc.data() as Map<String ,dynamic>;
        List<dynamic> userIds = data['join_user_ids'];
        print(userIds);
  ...
  static Future<void> sendMessage({required String roomId,required String message}) async {
    try {
      final messageCollection = _roomCollection.doc(roomId).collection(
          'message');
